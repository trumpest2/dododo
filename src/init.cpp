...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-loadblock=<file>", _("Imports blocks from external gaia000??.dat file on startup"));
    ...
    strUsage += HelpMessageOpt("-reindex", _("Rebuild chain state and block index from the gaia*.dat files on disk"));
#ifndef WIN32
...
{
    const std::string URL_SOURCE_CODE = "<https://gitlab.com/gaiacoin/gaiacoin>";
    const std::string URL_WEBSITE = "<https://gaia.baby/>";
    ...
    return FormatParagraph(strprintf("Copyright (C) %i-%i The Bitcoin Core Developers", 2009, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Gaiacoin Developers", 2014, 2018)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Gaiacoin Core Developers", 2018, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Please contribute if you find Gaiacoin Core useful. "
...
void CleanupBlockRevFiles()
{
    ...
    // Glob all gaia?????.dat and rev?????.dat files from the blocks directory.
    // Remove the rev files immediately and insert the gaia file paths into an
    // ordered map keyed by block file index.
    LogPrintf("Removing unusable gaia?????.dat and rev?????.dat files for -reindex with -prune\n");
    ...
    for (directory_iterator it(blocksdir); it != directory_iterator(); it++) {
        ...
        {
            if (it->path().filename().string().substr(0,3) == "gaia")
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("gaiacoin-loadgaia");
    CImportingNow imp;
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            if (!boost::filesystem::exists(GetBlockPosFilename(pos, "gaia")))
                break; // No block files left to reindex
            FILE *file = OpenBlockFile(pos, true);
            if (!file)
                break; // This error is logged in OpenBlockFile
            LogPrintf("Reindexing block file gaia%05u.dat...\n", (unsigned int)nFile);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Gaiacoin Core version %s\n", FormatFullVersion());
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Upgrading to 0.8; hard-link the old gaiannnn.dat files into /blocks/
    ...
    if (!boost::filesystem::exists(blocksDir))
    {
        ...
        for (unsigned int i = 1; i < 10000; i++) {
            boost::filesystem::path source = GetDataDir() / strprintf("gaia%04u.dat", i);
            if (!boost::filesystem::exists(source)) break;
            boost::filesystem::path dest = blocksDir / strprintf("gaia%05u.dat", i-1);
            try {
                ...
                LogPrintf("Error hardlinking gaia%04u.dat: %s\n", i, e.what());
                break;
            }
        }
        if (linked)
